...
//		cel[getRow(position)][getCol(position)].shoot();
	...
	public boolean lastShotWasHit(String position) {
		int r = getRow(position);
		int c = getCol(position);
		String key = createKey(r, c);
		Square target = map.get(key);
		return target.wasHit();
	}
	...
	public void isOneShipSunk() {
		for (int i = 0; i < ships.length; i++) {
			if (ships[i].isSunk()) {
				System.out.println(ships[i].getName() + ", is down | size: " + ships[i].getSize());
			}
